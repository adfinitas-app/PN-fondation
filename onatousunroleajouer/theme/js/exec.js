...
hongi.settings = {
	...
	twitterTexts: {
		...
		video: "#ONATOUSUNROLEAJOUER Découvrez le rôle de Kad Merad et Elsa Zylberstein aux côtés de Perce-Neige."
	},
...
proto.setHeadlineVideoContent = function(videoId){
	...
	player = hongi.app.ytPlayers['artists-yt-player'];
	wrapper = $('#artists-yt-player');
	
	// console.log('[setHeadline1Video] video: "' + videoId + '" in player: "artists-yt-player"');
	...
	$('[data-yt-target]').each(function(){
		var mi = $(this);
		if(mi.attr('data-yt-target') == 'artists-yt-player'){
...
proto.setHeadline = function(index){
	...
	if(this.currentHeadline == 0){
		this.ytPlayVideo(false, $('#artists-yt-player'));
	}
...
proto.setHeadlineTarget = function(isLargeStatus){
	...
	if(this.raceIsYTReady)
		hongi.app.ytInitPlayer('artists-yt-player', hongi.settings.defaultVideoId);
	$('[data-yt-target]').each(function(){
		var mi = $(this);
		if(mi.attr('data-yt-target') == 'artists-yt-player'){
			mi.toggleClass('active', mi.attr('data-yt-id') == hongi.settings.defaultVideoId);
		}
	});
};
...
proto.splashScreenOpen = function(){
	...
	this.splashScreenTl.add([
		...
		TweenLite.fromTo(
			...
			(hongi.settings.splashScreenDuration * .001) - 2.6,
			{ width: '0%' },
...
proto.ytInitPlayer = function(playerId, videoId){
	...
	console.log('playerId', playerId, 'videoId', videoId);
